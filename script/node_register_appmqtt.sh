...
fn_register_with_pattern() {
    ...
    hzn register --pattern "${HZN_ORG_ID}/pattern-${EDGE_OWNER}.${EDGE_DEPLOY}.appmqtt.yolo-${ARCH}" --input-file ../node/user-input-appmqtt.json --policy=../node/node_policy_privileged.json
}
...
fn_register_with_policy() {
    ...
    hzn exchange node create -n $HZN_EXCHANGE_NODE_AUTH
    hzn register --policy=../node/node_policy_appmqtt.json --input-file ../node/user-input-appmqtt.json
}
